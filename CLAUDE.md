...
- `Save.achievements` stays an array of unlocked ids. Schema is at `CURRENT_VERSION = 5`; the v3→v4 migration remaps the old flat achievement ids onto the tiered scheme (recomputing tier unlocks from the persisted lifetime stats, no unlocks lost), and v4→v5 backfills character skins. `CFG.achievements.pointsPerLevel` tunes the points→level curve.
...
- **Character skins** (`src/skins.js`, `SKINS`/`SKINS_BY_ID`) are coin-bought cosmetic player-body swaps. The Store has a third **SKINS** tab (flat list sorted cheapest→most expensive; buy auto-equips, owned skins one-tap EQUIP); `LoadoutScene` has a fifth **SKIN** slot. Owned/equipped state lives in `Save.ownedSkins` and `Save.loadout.skin` (the `v4 → v5` migration backfills `['default']`/`'default'`). The default skin (`default`/"Recruit", price 0, always owned) keeps the original `assets/player/body/` art. A skin is a full alternate set of the 48 player frames under `public/assets/player/skins/<id>/<dir>-<pose>.png`; `GameScene.playerFrameKey()` resolves to the equipped skin's frame when `skin.assetsReady` and the texture exists, else falls back to the default frame per-pose. **All seven non-default skins (ninja, girl/Ranger, panda, robot/Mech, knight, pirate, astronaut) now ship full frame sets and are `assetsReady: true`.** Store rows/preview and the loadout SKIN slot show each skin's own `south-idle` thumbnail via `skinThumb()`. The `tint` field is retained only as the placeholder look for any future skin added before its art lands — set such a skin's `assetsReady` to `false` until then. Generation prompts/paths/sizes: `plan_docs/skin-image-prompts.md`.
- **Boss checkpoints:** clearing a boss wave records a persistent checkpoint (`Save.progress.checkpointWave`, set via `Save.setCheckpointWave(this.wave)` on confirmed boss defeat in `GameScene.onEnemyKilled('boss')` — monotonic, boss-wave-validated, capped at `CFG.boss.finalWave`). When `checkpointWave > 0`, `MainMenuScene` offers **CONTINUE** (resume at `checkpointWave + 1`) and **NEW GAME** (fresh wave-1 run, which keeps the checkpoint). The start wave flows via `scene.start('GameScene', { startWave })`; `GameScene.create` reads `data.startWave` (default 1) and `beginAtWave()` is shared with the `jumpToWave` cheat.
- Every `CFG.boss.everyNWaves`th wave (default 10) is a boss wave: `GameScene.startBossWave()` spawns a large, 3-phase, shielded boss (orbiting weak points, summons its own minions) instead of the normal spawn cadence. Each boss wave uses a different archetype from `CFG.boss.variants` (`bossVariant(tier)`), with distinct colours/powers and difficulty scaling with the wave; the final boss is **The Annihilator** at `CFG.boss.finalWave` (100). Bosses can only summon enemy types whose `appearFromWave` is below the boss's wave (`bossSummonPool`). The boss is created only by `startBossWave()` and is not in `ENEMY_TYPE_ORDER`. Art-backed variants set an `id` and register frames in `BOSS_SPRITES`; generated boss art now covers Warden through Voidcaller, while remaining later bosses retain the primitive fallback until their sprite sets land. Per-wave sprite-generation prompts are in `plan_docs/boss-image-prompts.md` and `plan_docs/boss-retro-art-prompts.md` (overall design in `plan_docs/plan-boss.md`).
