...
- The wave-10 Warden and wave-20 Juggernaut each have complete 21-frame 256×256 sprite sets under `public/assets/enemies/boss/`; `BOSS_SPRITES` in `src/enemies.js` registers their directional idle, walk, power, enrage, and death frames.
...
- **Variants & scaling:** `CFG.boss.variants[]` holds 10 archetypes (index 0 = wave 10 … 9 = wave 100); `bossVariant(tier)` clamps, so tiers past 10 repeat the final boss. Each variant defines `name`, palette (`body`/`accent`/`core`/`proj`), `weakPointsByPhase`, and `phasePowers` (which powers are active in each phase). Difficulty scales **continuously** with tier: HP (`baseHp`+`hpPerTier`), shield (`baseShield`+`shieldPerTier`), attack cadence (`bossCadence`), projectile speed (`bossProjSpeed`), move speed, and weak-point orbit speed all grow with tier.
- **Rendering:** the wave-10 Warden and wave-20 Juggernaut each use a 21-frame 256×256 art set registered in `BOSS_SPRITES`: eight directional idle poses, a south-facing walk loop, per-power telegraph/release poses, phase-3 enrage, and a four-frame death sequence. `updateBossSprite` selects movement/facing frames and `playBossPowerArt` delays art-backed casts through their telegraphs before release; the Juggernaut's charge frames lock across the existing windup and lunge states. Other variants retain the palette-tinted `Arc` fallback. In both paths, the shield ring and orbiting weak points remain independent code-drawn overlays. A dedicated top-screen HP/shield bar (`bossBar*` HUD) shows the variant name + red HP fill + cyan shield gauge + phase. Per-wave sprite-generation prompts live in `plan_docs/boss-retro-art-prompts.md`.
...
- **Death:** `damageEnemy → killBoss → onEnemyKilled('boss')` pays the scaled coin reward (`payBossReward`), plays a death burst, clears remaining adds (`clearBossAdds` if `clearAddsOnBossDeath`), and tears down boss state/HUD (`teardownBossState`). `jumpToWave` also calls `teardownBossState`.

The boss is **not** in `ENEMY_TYPE_ORDER`, so `pickEnemyType()` never spawns it during normal waves; it is only created by `startBossWave()`. The bestiary uses real south-facing Warden and Juggernaut portraits.
