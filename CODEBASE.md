...
- **Rendering:** the wave-10 Warden uses its 21-frame 256×256 art set registered in `BOSS_SPRITES`: eight directional idle poses, a south-facing walk loop, summon/barrage telegraph and release poses, phase-3 enrage, and a four-frame death sequence. `updateBossSprite` selects movement/facing frames and `playWardenPower` delays each cast through its telegraph before release. Other variants retain the palette-tinted `Arc` fallback. In both paths, the shield ring and orbiting weak points remain independent code-drawn overlays. A dedicated top-screen HP/shield bar (`bossBar*` HUD) shows the variant name + red HP fill + cyan shield gauge + phase. Per-wave sprite-generation prompts live in `plan_docs/boss-retro-art-prompts.md`.
...
The boss is **not** in `ENEMY_TYPE_ORDER`, so `pickEnemyType()` never spawns it during normal waves; it is only created by `startBossWave()`. The bestiary entry in `src/enemies.js` uses the Warden's real south-facing portrait.
