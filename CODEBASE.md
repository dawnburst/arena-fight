...
The boss is **not** in `ENEMY_TYPE_ORDER`, so `pickEnemyType()` never spawns it during normal waves; it is only created by `startBossWave()`. The bestiary uses real south-facing Warden, Juggernaut, Hexweaver, Bombardier, Phantom, Overlord, Tempest, and Colossus portraits.
