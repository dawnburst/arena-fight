...
## Current update note — 2026-06-09
...
- **Composite (primitive, Phase 1):** the boss "enemy" is a large `Arc` body (visual radius `CFG.boss.radius`, larger physics `hitRadius` covering the orbiting weak points), tinted with the variant palette. Code-drawn overlays follow it each frame: a translucent `shieldRingGfx`, a bright `core`, and small orbiting weak-point nodes (`weakPoints`). A dedicated top-screen HP/shield bar (`bossBar*` HUD) shows the variant name + red HP fill + cyan shield gauge + phase. Per-wave sprite-generation prompts live in `plan_docs/boss-image-prompts.md`.
