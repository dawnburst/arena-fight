...
- The player-held weapon overlay now swaps per equipped weapon. `GameScene.preload()` loads the two equipped held sprites from `public/assets/player/weapons/<weaponId>.png` with texture keys `weapon-held-<weaponId>`; `setBarrelTexture()` falls back to `player-rifle` (`public/assets/player/rifle.png`) if a dedicated texture is missing.
- The wave-10 Warden and wave-20 Juggernaut each have complete 21-frame 256×256 sprite sets, the wave-30 Hexweaver and wave-40 Bombardier each have 23 frames, the wave-50 Phantom, wave-60 Overlord, wave-70 Tempest, and wave-80 Colossus each have 25 frames, the wave-90 Voidcaller has 27 frames, and the wave-100 final boss Annihilator has 31 frames under `public/assets/enemies/boss/`; `BOSS_SPRITES` registers their directional idle, movement, power, enrage, and death frames.
...
- `updateAim()` computes `this.aimAngle = atan2(pointerY - py, pointerX - px)`. Sets `barrel.rotation`; the held weapon texture follows the active weapon and flips vertically when aiming left.
...
- Held weapon art: `spawnPlayer()` creates the shared barrel overlay, then `setBarrelTexture(this.weaponDef)` swaps it to `weapon-held-<id>` when loaded. `switchWeapon()` calls the same helper after changing `this.weaponDef`. Dedicated sprites are `128x48` transparent PNGs in `public/assets/player/weapons/`; missing textures fall back to `player-rifle`.
...
The boss is **not** in `ENEMY_TYPE_ORDER`, so `pickEnemyType()` never spawns it during normal waves; it is only created by `startBossWave()`. The bestiary uses real south-facing Warden, Juggernaut, Hexweaver, Bombardier, Phantom, Overlord, Tempest, Colossus, Voidcaller, and Annihilator portraits.
...
3. Add a `128x48` transparent held-weapon sprite at `public/assets/player/weapons/<id>.png`, pointing right with the grip aligned to `public/assets/player/rifle.png`. If omitted, gameplay falls back to the rifle overlay.
4. No further integration needed — Store and Loadout pick it up automatically (assuming `price > 0`, which makes it appear in the Store).
