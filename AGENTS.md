...
- `src/skins.js` contains the character-skins catalog (`SKINS`) — coin-bought cosmetic player-body swaps. Owned/equipped state is `Save.ownedSkins`/`Save.loadout.skin` (`v4 → v5` migration). All seven non-default skins now ship full per-skin frame sets under `public/assets/player/skins/<id>/<dir>-<pose>.png` and are `assetsReady: true` (the `tint` placeholder remains only for any future skin added before its art lands); prompts in `plan_docs/skin-image-prompts.md`.
